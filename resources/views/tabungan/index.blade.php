                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="text-center">
                                {{-- Tombol Cetak --}}
                                <a class="icon" href="{{ route('tabungan.transaksicetak', $item->id) }}" target="_blank" title="cetak transaksi">
                                    <i class="fe fe-printer"></i>
                                </a>

                                {{-- Tombol Hapus --}}
                                <a class="icon btn-delete" href="#!" data-id="{{ $item->id }}" title="hapus transaksi">
...
<script>
    $(document).ready(function() {
        ...
        $('#siswa').on('change', function() {
            if (this.value == '#') {
                $('#saldo').text('0');
                $('#form-jumlah, #form-keterangan, #submit').hide();
                return;
            }
            ...
            $.ajax({
                url: "{{ route('api.getsaldo') }}/" + siswa_id,
                beforeSend: function() {
                    $('#form-jumlah, #form-keterangan, #submit').hide();
                ...
                success: function(result) {
                    ...
                    saldo = result.saldo;
                    $('#form-jumlah, #form-keterangan, #submit').show();
                },
                error: function() {
                    $('#saldo').text('Gagal mengambil saldo');
                }
            });
        });
        ...
        $('#submit').on('click', function() {
            if ((saldo <= 0 && keperluan === 'out') || (saldo < jumlah && keperluan === 'out')) {
                swal({
                    title: 'Tidak dapat melakukan penarikan',
                    text: 'Saldo: ' + saldo + ', Jumlah: ' + jumlah,
                    dangerMode: true,
                });
                return;
            }

            if (jumlah <= 99 || isNaN(jumlah)) {
                swal({
                    title: 'Jumlah belum diisi dengan benar',
                    dangerMode: true,
                });
                return;
            }
            ...
            $.ajax({
                ...
                error: function(data) {
                    console.log('Error saat submit:', data);
